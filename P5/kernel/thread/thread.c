...
void thread_schedule()
{
    ...
    pre_running = current_running[mhartid];
    ...
    current_running[mhartid] = list_to_pcb(p);
    leave_out_list(p); 
    current_running[mhartid]->status = TASK_RUNNING;
    process_id = current_running[mhartid]->pid;
    
    switch_to(pre_running, current_running[mhartid]);
}
...
void init_thread_pcb(uint64_t func_entry, int thread_rank)
{
    ...
    ptr_t kernel_stack = 0;
    ptr_t user_stack   = 0;
    pcb_count++;
    pcb[pcb_count].pid    = current_running[mhartid]->pid;
