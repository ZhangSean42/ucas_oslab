...
void interrupt_helper(regs_context_t *regs, uint64_t stval, uint64_t scause)
{
    ...
    if (current_running[get_current_cpu_id()]->killed){
        do_exit();
    }
...
void init_exception()
{
    ...
    exc_table[EXCC_INST_PAGE_FAULT]  = (handler_t)inst_page_fault;
    exc_table[EXCC_LOAD_PAGE_FAULT]  = (handler_t)load_page_fault;
    exc_table[EXCC_STORE_PAGE_FAULT] = (handler_t)store_page_fault;
