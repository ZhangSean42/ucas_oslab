...
void sys_mkfs()
{
    invoke_syscall(SYSCALL_MKFS, 0, 0, 0, 0, 0);
}

void sys_statfs(char *buffer)
{
    invoke_syscall(SYSCALL_STATFS, buffer, 0, 0, 0, 0);
}

int sys_enter_fs(char *path)
{
   return invoke_syscall(SYSCALL_ENTER_FS, path, 0, 0, 0, 0);
}

int sys_mkdir(char *path)
{
   return invoke_syscall(SYSCALL_MKDIR, path, 0, 0, 0, 0);
}

int sys_rmdir(char *path)
{
   return invoke_syscall(SYSCALL_RMDIR, path, 0, 0, 0, 0);
}

int sys_ls(char *path, char *buffer, int ls_l)
{
   return invoke_syscall(SYSCALL_LS, path, buffer, ls_l, 0, 0);
}

int sys_create(char *path)
{
   return invoke_syscall(SYSCALL_CREATE, path, 0, 0, 0, 0);
}

int sys_cat(char *path)
{
   return invoke_syscall(SYSCALL_CAT, path, 0, 0, 0, 0);
}

void sys_get_cwdpath(char *buffer)
{
    invoke_syscall(SYSCALL_GET_CWDPATH, buffer, 0, 0, 0, 0);
}

int sys_fopen(char *path, int access)
{
    return invoke_syscall(SYSCALL_FOPEN, path, access, 0, 0, 0);
}

int sys_fread(int fd, char* buffer, int sz)
{
    return invoke_syscall(SYSCALL_FREAD, fd, buffer, sz, 0, 0);
}

int sys_fwrite(int fd, char* buffer, int sz)
{
    return invoke_syscall(SYSCALL_FWRITE, fd, buffer, sz, 0, 0);
}

void sys_fclose(int fd)
{
    invoke_syscall(SYSCALL_FCLOSE, fd, 0, 0, 0, 0);
}

int sys_lseek(int fd, int offset, int whence)
{
    return invoke_syscall(SYSCALL_LSEEK, fd, offset, whence, 0, 0);
}

int sys_ln(char *fil_path, char *dir_path)
{
    return invoke_syscall(SYSCALL_LN, fil_path, dir_path, 0, 0, 0);
}
/************************************************************/