...
static void create_image(int nfiles, char *files[])
{
    ...
    for (int fidx = 0; fidx < nfiles; ++fidx) {
        ...
        if (taskidx >= 0){
            memcpy(taskinfo[taskidx].app_name, *files, strlen((char*)*files));
            taskinfo[taskidx].app_name[strlen((char*)*files)+1] = 0;
    ...
    for (int fidx = 0; fidx < nfiles; ++fidx) {
        ...
        for (int ph = 0; ph < ehdr.e_phnum; ph++) {
            ...
            if (strcmp(*files, "main") != 0){
                ...
                if (taskidx >= 0 && phdr.p_memsz != 0)
                    taskinfo[taskidx].mem_sz = phdr.p_memsz;
                write_segment(phdr, fp, img, &phyaddr);
            }
            else{
...
static void write_img_info_vm(int nbytes_kernel, int nbytes_decompress, int app_info_offs, task_info_t *taskinfo, short tasknum, FILE * img)
{
    ...
    for (int i = 0; i < tasknum; i++){
        printf("\tapp_name %s\n", taskinfo[i].app_name);
        printf("\t\timg_offset 0x%04lx", taskinfo[i].app_img_offs);
        printf("\t\tfile_size 0x%04lx\n", taskinfo[i].app_sz);
        printf("\t\tmem_size 0x%04lx\n", taskinfo[i].mem_sz);
    }
}
