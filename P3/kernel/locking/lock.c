...
int do_mbox_open(char *name)
{
    ...
    for (int i = 0; i < MBOX_NUM; i++){
        if (mboxs[i].key[0] == '\0'){
            ...
            mboxs[i].usnr++;
            mboxs[i].send_lock.status = UNLOCKED;
            mboxs[i].recv_lock.status = UNLOCKED;
            mboxs[i].psend = 0;
            mboxs[i].precv = 0;
...
{
    int block = 0;
    spin_lock_acquire(&mboxs[mbox_idx].send_lock);
    while (mboxs[mbox_idx].psend + msg_length > MAX_MBOX_LENGTH){
        block = 1;
        do_block(&current_running[get_current_cpu_id()]->list, &mboxs[mbox_idx].send_block_queue, &mboxs[mbox_idx].send_lock);
    }
    for (int i = 0; i < msg_length; i++){
        mboxs[mbox_idx].mbox_buf[mboxs[mbox_idx].psend++] = (*(char*)(msg+i));
    }
    spin_lock_release(&mboxs[mbox_idx].send_lock);
    
    spin_lock_acquire(&mboxs[mbox_idx].recv_lock);
    ...
    spin_lock_release(&mboxs[mbox_idx].recv_lock);
...
int do_mbox_recv(int mbox_idx, void * msg, int msg_length)
{
    int block;
    spin_lock_acquire(&mboxs[mbox_idx].send_lock);
        if (mboxs[mbox_idx].psend == mboxs[mbox_idx].precv){
            mboxs[mbox_idx].psend = 0;
            mboxs[mbox_idx].precv = 0;
            ...
            spin_lock_acquire(&mboxs[mbox_idx].recv_lock);
            block = 1;
            add_to_list(&mboxs[mbox_idx].recv_block_queue, &current_running[get_current_cpu_id()]->list);
...
recv:
    for (int i = 0; i < msg_length; i++){
        *((char*)(msg+i)) = mboxs[mbox_idx].mbox_buf[mboxs[mbox_idx].precv++];
    }
